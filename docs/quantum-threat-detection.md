...
## Signal-to-Tier Matrix

| Signal Class | Example Trigger | Typical Tier |
| --- | --- | --- |
| Calibration drift | Δ frequency > Nσ for 2 windows | 1–2 |
| Queue saturation | queue depth + retry spike | 1–2 |
| Network jitter | tail latency growth > Nσ | 1–2 |
| Cross-domain correlation | drift + jitter + retries | 2–3 |
| Integrity failures | signature fails + correlation | 4 |
...
## Response Workflow (Runbook Summary)

1. **Triage**: confirm data quality and baseline freshness.
2. **Correlate**: validate cross-domain signals with aggregate telemetry.
3. **Classify**: assign tier and confidence.
4. **Document**: record window, signals, and decision rationale.
5. **Review**: human review for Tier 3–4 events.
...
- Retain aggregate feature windows only for the minimum policy period.
- Delete raw event fields once features are computed.
- Rotate salts for hashes per reporting period.
