"""ETL skeleton for NASA_BlackMarble_VNP46A2.
...
def run(config):
    ...
    out = Path(config.get("out_dir", "data")) / "ntl_deltas.jsonl"
    out.parent.mkdir(parents=True, exist_ok=True)
    # Placeholder example record
    record = {"source":"NASA_BlackMarble_VNP46A2","status":"placeholder","ts":time.time()}
